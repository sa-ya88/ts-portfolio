...
export default function Hire() {
  return (
    <main className="mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
      ...
      <h1 className="mt-2 text-3xl font-bold tracking-tight text-ink">仕事の依頼について</h1>
      <p className="mt-4 leading-relaxed text-ink/70">
        リモート中心で、個人の業務委託として受けています。金額はすべて税別の目安です。範囲が決まっていなくても、まずはご相談ください。
      </p>

      <section className="mt-10">
        <h2 className="text-lg font-bold text-ink">料金目安</h2>
        <div className="mt-4 space-y-3">
          {prices.map((item) => (
            <article key={item.title} className="card-pop p-5">
              <div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between">
                <h3 className="font-bold text-ink">{item.title}</h3>
                <p className="text-sm font-bold text-coral">{item.price}</p>
              </div>
              <p className="mt-2 text-sm leading-relaxed text-ink/60">{item.note}</p>
            </article>
          ))}
        </div>
      </section>
      ...
      <section className="mt-10">
        <h2 className="text-lg font-bold text-ink">進め方</h2>
        <ol className="mt-4 space-y-3">
          {steps.map((step, index) => (
            <li key={step.title} className="card-pop flex gap-4 p-5">
              <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-coral/12 text-sm font-bold text-coral-dark">
                {index + 1}
              </span>
              <div>
      ...
      <section className="mt-10 grid gap-4 sm:grid-cols-2">
        <div className="card-pop border-l-4 border-l-mint p-5">
          <h2 className="font-bold text-ink">対応しやすいこと</h2>
        ...
        <div className="card-pop border-l-4 border-l-coral p-5">
          <h2 className="font-bold text-ink">事前にご相談ください</h2>
          <ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-ink/75">
